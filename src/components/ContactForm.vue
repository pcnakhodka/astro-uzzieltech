<template>
    ...
    <div class="grid gap-x-4 gap-y-1 p-2 my-4">
      ...
      <div class="relative w-full mt-2">
        ...
        <input type="text" class="border-0 px-3 py-3 bg-white dark:bg-black rounded text-sm shadow-md focus:outline-none focus:ring w-full ease-linear transition-all duration-150" required placeholder="+254 712 345 678" v-model="form.phone">
      </div>
      <div class="relative w-full mt-2">
        <label
          class="block uppercase text-xs font-bold mb-2"
      ...
      <button class="bg-blue-600 text-white p-2 rounded-lg mt-2 flex space-x-2 place-content-center font-semibold" v-if="!contacted">
  ...
  methods:{
    contact(){
      ...
      fetch(`${this.url}items/contact`, {
        ...
        this.response = "Message received. I will contact you soon"
        this.contacted = true
      }).catch(() => {
        this.loading = false
        this.response_type = false
        this.response = "Unable to send message at this moment"
